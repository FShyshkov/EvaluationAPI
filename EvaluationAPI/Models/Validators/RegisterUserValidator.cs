...
namespace EvaluationAPI.Models.Validators
{
    public class RegisterUserRequestValidator : AbstractValidator<RegisterUserRequest>
    {
        ...
        {
            RuleFor(x => x.FirstName).Length(2, 30).NotEmpty().Matches(@"^[a-zA-Z]+$");
            RuleFor(x => x.LastName).Length(2, 30).NotEmpty().Matches(@"^[a-zA-Z]+$");
            RuleFor(x => x.Email).EmailAddress().NotEmpty().Matches(@"^[a-zA-Z]+$");
